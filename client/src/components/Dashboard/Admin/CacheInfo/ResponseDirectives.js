...
const ResponseDirectives = () => {
      ...
      if (isMounted) {
        ...
        const chartInfo = {
          data: res[0].data.map((item) => ({
            ...
            data: item.cacheabilityDirectivesArray,
          })),
        };
  ...
  useEffect(() => {
    const ctx = document
      .getElementById('responseDirectivesChart')
      .getContext('2d');
  ...
  return (
    <>
      <h3> Response Directives Percentages</h3>
      <div>
      ...
      <canvas id="responseDirectivesChart" width="225" height="125"></canvas>
    </>
  );
};

export default ResponseDirectives;
